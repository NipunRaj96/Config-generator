...
_JOB_KEYWORDS: frozenset[str] = frozenset({
    "job", "career", "opening", "position", "positions", "apply", "vacancy", "hiring",
...
class TrafficInterceptor(PipelineStep):
    ...
    def _capture_and_continue(self, url: str, state: PipelineState) -> StepResult:
        ...
        captured, page_html, html_candidates = self.capture(url, state)
    ...
    def capture(
        self, career_url: str, state: PipelineState
        ...
        def _on_response(response) -> None:
            try:
                ...
                response_statuses[url] = response.status
            except Exception as exc:
        ...
        # Locate and click visible pagination/Load More/Next Page buttons to trigger XHR/fetch requests
        accumulated_html = []
        try:
            accumulated_html.append(page.content())
        except Exception:
            pass

        try:
            # Common patterns for load more buttons / next page links / page numbers
            selectors = [
                ...
                # Next Page / Pagination selectors
            ...
            while click_count < max_clicks:
                ...
                for selector in selectors:
                    ...
                    for j in range(count):
                        ...
                        if el.is_visible() and el.is_enabled():
                            # Check if the element or its parent is disabled
                            class_attr = el.get_attribute("class") or ""
                            parent_class = page.evaluate("el => el.parentElement ? el.parentElement.className : ''", el)
                            if "disabled" in class_attr.lower() or "disabled" in parent_class.lower():
                                continue

                            logger.info("TrafficInterceptor: Clicking pagination element: %s (click %d)", selector, click_count + 1)
                            try:
                                el.click(timeout=5000)
                                page.wait_for_timeout(3000)
                                # Append the new HTML content after pagination action
                                accumulated_html.append(page.content())
                                clicked_any = True
                                click_count += 1
                            except Exception as click_err:
                                logger.debug("Click failed: %s", click_err)
                            break
                    if clicked_any:
                ...
                if not clicked_any:
                    # No pagination/load more elements were visible and clickable in this scan
                    break
        ...
        state.pagination_detected = (click_count > 0)

        # ── Capture rendered HTML (after JS execution) ──────────────────────────
        page_html: Optional[str] = None
        try:
            if len(accumulated_html) > 1:
                # Wrap each page in a div to ensure a single parsed root tree
                page_html = "<div>" + "</div><div>".join(accumulated_html) + "</div>"
            else:
        ...
        enriched: list[CapturedRequest] = [
            req.model_copy(update={
                ...
                "response_status": response_statuses.get(req.url, 0),
            })
            for req in captured
        ]
        ...
        for req in enriched:
            ...
            # Exclude JSON/prefetch endpoints from the HTML/LOCRGX candidate pool
            ...
            is_json_like = (
                ...
                "_next/data/" in url_lower or
                "page-data" in url_lower
            )
            if is_json_like:
                continue
    ...
    def close(self) -> None:
        """Explicitly shut down the browser singleton. Call at application exit."""
    ...
    def _get_browser(self):
        from src.browser_manager import BrowserSessionManager
        return BrowserSessionManager.get_browser()

...
> **Last updated: 2026-07-16 (v6 — Dynamic Pagination Limits + Multi-key rotation + Playwright Headless + Proxy Bypass via HTML Cleanup/Capping + Semantic XPath Pagination Skipping)**

---

## 1. What This Project Does (and Does NOT Do)

### The Problem & TechOps Workflow
TechOps engineers at Naukri manually analyse a company's career page to identify how jobs are loaded:
1. **Navigate to Career Page:** Go to the target career site URL.
2. **Open Network Tab:** Open Chrome DevTools (F12) and go to the **Network** tab.
3. **Search for Job Content:** Open the search pane in DevTools Network tab (Ctrl+F / Cmd+F inside the network pane) and enter a specific job title currently visible on the page (e.g. "Assistant Manager").
4. **Identify the Source Request:** Click through the search results to find the underlying network request/API endpoint whose response payload contains the target job title.
5. **Inspect Headers:** Check the headers of that request to find the exact request URL (Integration Link), HTTP method, query parameters, request body, and headers.
6. **Determine Config Path:** 
   - **If the response is JSON:** A JPERL `LOCJSON` config is created based on object path mapping.
   - **If the response is HTML:** A JPERL `LOCRGX` config is created using PCRE regular expressions.

This manual process takes 15–30 minutes per site. This tool automates this workflow end-to-end.
...
└──────────┬──────────────────┘
           ...
           ▼
┌─────────────────────────────┐
│  4. TrafficInterceptor      │ → Opens Playwright Chromium (headless=True)
│                             │   Captures: XHR/Fetch requests → state.candidates
│  HALT_FAIL if 0 requests    │   Click-loops "Load More" / "Next" (up to 10 times) to load all jobs.
│  CONTINUE otherwise         │   [v6] Sets state.pagination_detected = True if pagination clicked.
└──────────┬──────────────────┘
           │
           ▼
┌─────────────────────────────┐
...
│                             │   [v6] Boilerplate stripping (script, style, SVG, header, footer, nav)
│  CONTINUE if regex fails    │   [v6] Truncates DOM context window strictly to 30,000 characters
│  HALT_OK if regex validated │   (bypasses corporate proxy payload limits & prevents zero-replies).
│  HALT_FAIL if no HTML at all│   [v6] Anchor weights: demotes tables/ul to 10, class/id match to 90.
└──────────┬──────────────────┘
           │
           ▼
┌─────────────────────────────┐
...
│                             │   [v6] Skips if state.pagination_detected=True OR expected_jobs > 20
│  HALT_OK always             │   to force JPERL (Regex) on paginated sites.
│                             │   [v6] Validation: invalidates XPath if match_count < expected_jobs.
└──────────┬──────────────────┘
           │
           ▼
┌─────────────────────────────┐
...
## 8. New Steps in Detail (v6)

### LOCRGXGenerator (src/locrgx_generator.py)
...
**Boilerplate Stripping (Context Optimization):**
To ensure the LLM receives only clean HTML structure and fits the tokens well within enterprise proxy payload filters, we strip:
...
- Navigation: `<nav>...</nav>`
This typically cuts the raw HTML character weight by **50% to 80%**.

**Anchor-Based Trimming:**
- Searches the cleaned HTML for specific patterns that anchor around the job listings.
- **Anchor Weights System**: Demotes generic tags like `<ul>` and `<table>` to weight **10** (since they wrap the entire page and cause wrong context centering). Promotes class/id selectors containing job terms (`job`, `career`, `opening`, `position`, `listing`, `vacancy`) to weight **90** (forcing accurate job container centering).
- Takes a slice of **30,000 characters** starting 2,000 characters before the highest-weighted match index. This keeps the prompt size under **30KB**, bypassing corporate proxy payload limit blocks which intercept calls to `generativelanguage.googleapis.com` and spoof them with strings of zeros (`000000...`).
...
**Note on HTML Entities:** HTML entities in captured HTML (like `&#34;` for `"`) are automatically decoded using `html.unescape()` before prompt construction and validation to prevent LLM regex syntax mismatches.


---

### XPathSRPGenerator (src/xpath_srp_generator.py)

**Fires for:** ONLY when `state.is_srp == True`
**Skip condition:** if `state.is_srp == False` → `CONTINUE` immediately

**Smart Skip for Paginated Sites (Forced JPERL):**
XPath configs are not preferred for paginated pages since only the JPERL crawler supports dynamic URL parameter substitutions. To enforce JPERL (Regex) on paginated sites:
- **Playwright Interceptor Flag**: If Playwright successfully finds and clicks pagination/load-more elements in the browser (`state.pagination_detected == True`), we immediately skip XPath.
- **Expected Jobs Threshold**: If the expected job count is greater than 20 (`inp.jobs_on_career_page > 20`), we skip XPath.
- **Mathematical Pagination Validation**: If the generated XPath matches fewer jobs on the page than the total expected job count (`match_count < inp.jobs_on_career_page`), it indicates pagination exists. The XPath is dynamically invalidated, causing the generator to fail and fall back to JPERL (Regex).
- If all checks pass (i.e. single-page site with 15–20 jobs), XPath is allowed to run.
...
| **Run 8** | **2026-07-16** | **v6 — Dynamic Pagination page calculations, Playwright headless execution, HTML boilerplate stripping, anchor heuristic demotions, proxy bypass, mathematical pagination validation** | **5/5 (100%)** | **100%** |
...
| 2026-07-16 | **v6 — Dynamic Pagination limits, Playwright headless mode, HTML boilerplate stripping, anchor weight tuning, proxy payload bypass, mathematical pagination validation.** Integrated headless browser configurations, stripped SVG/style/script/header/footer tags, changed HTML slicing to 30,000 characters to bypass proxy blocks, tuned anchor scoring weights (90 for job cards, 10 for table/ul wrapper tags), enabled dynamic calculation of `MAXPAGESPARSE = ceil(total_jobs / per_page) + 5`, added `state.pagination_detected` to track Playwright paginated clicks, and implemented mathematical validation comparing page 1 job matches to expected site jobs to reject XPath in favor of JPERL on paginated sites. |
...
## 13. Telemetry, Split Reasoning, & Evaluation Framework (2026-07-10 Updates)

The pipeline has been upgraded with a comprehensive Telemetry Database, Split LLM Reasoning steps, and an automated Evaluation Framework.

### A. SQLite Telemetry Database (`knowledge_base/pipeline.db`)
Records deep operational metrics and reasoning traces for all non-cached runs:
* **`runs` Table**: Records company metrics, final crawler type, final status, and confidence scores.
* **`traces` Table**: Logs the exact `raw_prompt_api`, `raw_response_api`, `raw_prompt_fields`, and `raw_response_fields` along with the list of evaluated candidates and their response samples (`candidate_samples`).
* **`metrics` Table**: Logs execution time, playwright intercepts count, and LLM requests count.

### B. Two-Stage Split LLM Extraction (`src/llm_reasoner.py`)
To prevent reasoning errors, Call 1 handles **API & Jobs Path Selection** (enforcing a comparative Chain-of-Thought listing Pros/Cons for candidates).
Once verified, the jobs list is sliced to `jobs_list[:3]` and passed to Call 2 for **Field & Pagination Mapping**, reducing tokens and eliminating hallucination.

### C. Programmatic Confidence & Semantic Validation
Confidence is calculated programmatically (0.0 to 1.05) based on endpoint validity, job counts, title/ID uniqueness, location, and description mapping.
Dry-run checks reject configs with identical titles, duplicate IDs, or generic navigation titles in job fields.

### D. Offline Evaluation Framework (`evaluate.py`)
Executes `python evaluate.py` to evaluate overall accuracy. Uses local cached request bodies in `evaluation/known_sites.py` for Lever, Greenhouse, Workday, and custom JSON sites. Reports Endpoint, Jobs Path, Pagination, and Field Mapping segment accuracies.
